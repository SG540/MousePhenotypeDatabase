...
server <- function(input, output, session) {
  ...
  datasetInput <- reactive({
    ...
    if (input$task %in% c(
      ...
      dplyr::right_join(DBI::dbReadTable(connection(), input$task),
      ...
      dplyr::right_join(DBI::dbReadTable(connection(), paste0(input$task)),
                        mouse_info,by="MouseID") %>%
        left_join(., DBI::dbReadTable(connection(), paste0(input$task,"_sum")), by="MouseID")
      ...
      dplyr::right_join(DBI::dbReadTable(connection(), paste0(input$task)),
                        mouse_info,by="MouseID") %>%
        left_join(., DBI::dbReadTable(connection(), paste0("ps_sum_t02")), by="MouseID")
      ...
      dplyr::right_join(DBI::dbReadTable(connection(), paste0(input$task, "_t01")),
                        DBI::dbReadTable(connection(), paste0(input$task, "_sum_t01")),
                        by=paste0(toupper(input$task),"_TrialID")) %>%
        right_join(., mouse_info, by="MouseID")
      ...
      dplyr::right_join(DBI::dbReadTable(connection(), paste0(input$task,"_t01")),
                        mouse_info,by="MouseID") %>%
        left_join(., DBI::dbReadTable(connection(), paste0(input$task,"_sum_t01")), by="MouseID")
  ...
  output$plot <- renderPlot({
    ...
    val_ylab <- list(c("Body temperature (C)","Body weight (g)","Wire hang latency (sec)","Grip strength (N)"), #GHNS
                     ...
                     c("Time spent around cage (sec)","Distance travelled (cm)","Time spent around cage (sec)","Distance travelled (cm)"), #CrSoc
                     c("Distance traveled (cm)","Entries into open arms (%)","Time on open arms (ratio)","Number of entries"), #OF
                     c("Latency (sec)"), #RR
    ...
    if(input$task == "tm_t01"){
      tb <- filter(tb, TM_Time <= 5) %>%
        mutate(TM_PercentageOfCorrectResponses = TM_NumberOfCorrectResponses / TM_NumberOfFreeChoices * 100)
    } else if (input$task == "bm_t01"){
      tb <- filter(tb, BM_Time <= 15)
    } else if (input$task == "bm_probe"){
      tb <- filter(tb, str_detect(BM_probe_SubTrialNumber, "PT")) #not sure if correct
      ...
      tb <- pivot_longer(tb, contains("ASR"), names_to = "ASR", values_to = "PPI_ASR_value", names_prefix = "PPI_ASR") %>%
        ...
        mutate(PPI = str_replace(PPI, "_", "-")) %>%
        rename_at(vars(c(ASR,PPI)), ~ paste0("PPI_",c("ASR","PPI")))
    } else if (str_detect(input$task, "fz")){ 
      if (input$task == "fz_day1"){
        tb <- tb %>%
          mutate(FZ_Day1_Experiment = NA)
        tb2 <- filter(tb, FZ_Day1_Type=="Distance_Shock") %>%
          filter(!FZ_Day1_Time %in% c(24,48) & !is.na(FZ_Day1_DV)) %>% # I think that this part should be treated when making database
          mutate(FZ_Day1_Time = case_when(FZ_Day1_Time>24 & FZ_Day1_Time<48 ~ as.numeric(FZ_Day1_Time - 1), 
                                          FZ_Day1_Time>48 ~ as.numeric(FZ_Day1_Time - 2),
                                          TRUE ~ as.numeric(FZ_Day1_Time)), 
                 FZ_Day1_Shock = case_when((FZ_Day1_Time-1) %/% 23 == 0 ~ "Shock 1",
                                           (FZ_Day1_Time-1) %/% 23 == 1 ~ "Shock 2",
                                           (FZ_Day1_Time-1) %/% 23 == 2 ~ "Shock 3",
                                           TRUE ~ FZ_Day1_Type)) %>%
          mutate(FZ_Day1_Time = case_when((FZ_Day1_Time-1) %/% 23 == 0 ~ as.numeric(as.numeric(FZ_Day1_Time)/4+0.25),
      ...
      tb1 <- select(tb, c("MouseID","Genotype",paste0(prefix,c("_Type","_Experiment","_Time","_DV")))) %>%
        filter(get(paste0(prefix,"_Type")) != "Distance_Shock") %>% 
        filter(get(paste0(prefix,"_Time")) <= 8) %>%
        # group_by(MouseID) %>%
      ...
      tb <- tb %>%
        ...
        mutate(CrSoc_Place = case_when(str_detect(CrSoc_Place, "Left") ~ "Left", #"Empty side",
        ...
        mutate(CrSoc_Experiment = if_else(str_detect(CrSoc_SubID, "1"), "Sociability test", "Preference test"),
        ...
        filter(CrSoc_Type == "CrSoc_STofCage") %>%
        select(!c("CrSoc_NEinArea","CrSoc_STofArea","CrSoc_NEinCage")) %>%
        distinct()
    }
        ...
        myplots[[i]] <- local({
          ...
          if (xaxis == "Genotype") {
            ...
            if (input$task == "fz_day1") { #day1
              ...
              if(i==1 | i==3){#context
                tb1 <- tb1 %>%
                  filter(get(paste0(prefix,"_Experiment")) == "Context")
                ...
                tb1 <- tb1 %>%
                  filter(get(paste0(prefix,"_Experiment")) == "Cued")
            ...
            Plot <- 
              ...
              annotate("rect", xmin = 1, xmax = 12, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              annotate("rect", xmin = 25, xmax = 36, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              annotate("rect", xmin = 49, xmax = 60, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              annotate("rect", xmin = 73, xmax = 84, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              annotate("rect", xmin = 97, xmax = 108, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              annotate("rect", xmin = 121, xmax = 132, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              annotate("rect", xmin = 145, xmax = 156, ymin = -Inf, ymax = Inf,  fill = "lightgrey", alpha=.6) +
              stat_summary(aes(group=Genotype, colour=Genotype), fun=mean, geom="line", size=1) + 
              # annotate("text", x=1,y=60,label=paste(italic(P), "=", stat.test$p)) +
              # stat_compare_means(aes(group = Genotype), label = "p.signif") +
              scale_color_viridis_d(label=labels) +
            ...
            if(i < 3){
              tb1 <- select(tb, c("MouseID","Genotype",paste0(prefix,c("_SubID","_Type","_Experiment")),xaxis[i],List_var[i])) %>%
                filter(CrSoc_Experiment == "Sociability test")
            } else{
              tb1 <- select(tb, c("MouseID","Genotype",paste0(prefix,c("_SubID","_Type","_Experiment")),xaxis[i],List_var[i])) %>%
                filter(CrSoc_Experiment == "Preference test")
            }
            if(xaxis[i] == "Genotype"){
              tb1 <- select(tb1, !c(paste0(prefix,c("_Type","_Experiment")))) %>%
                distinct()
              stat.test <- tb1 %>%
              ...
              Plot <- 
                ggbarplot(tb1, x="Genotype", y=List_var[i], fill=xaxis[i], alpha=0.8, 
                          add = c("mean_se","jitter"), position = position_dodge(0.8)) +
                geom_jitter(aes(x=get(xaxis[i]), y=get(List_var[i])), col = "black", width = 0, alpha=.6) +
                stat_pvalue_manual(stat.test, label = "p", tip.length = 0.01, vjust=-0.5) +
                scale_fill_viridis_d(label=labels) +
                scale_y_continuous(expand = c(0, 0)) +
              ...
              # tb1 <- tb1 %>% mutate(!!xaxis[i] := factor(get(xaxis[i]), levels=sort(unique(tb1[[xaxis[i]]]))))
            ...
            # stat.test <- tb %>% #obtain p-values
            #   group_by(Genotype) %>%
            #   pairwise_t_test(List_var[i] ~ xaxis, paired = TRUE, p.adjust.method = "bonferroni")
            #   
            #   # anova_test(dv = List_var[i], wid = MouseID, within = eval(xaxis))
            #   # t_test(as.formula(paste0(List_var[i], " ~ Genotype"))) %>%
            #   add_xy_position(x = "Genotype", scales = "free_y")
            ...
            Plot <- 
              ...
              # scale_x_continuous(breaks=seq(0,max(tb[xaxis]),max(tb[xaxis])/6)) +
              scale_color_viridis_d(label=labels) +
              xlab(xlab) +
              ylab(List_label[i]) +
              theme_classic() + 
