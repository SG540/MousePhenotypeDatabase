      ...
      for (i in seq_along(List_var)) {
        ...
        myplots[[i]] <- local({
          ...
          if (xaxis[1] == "Genotype") {
            ...
            Plot <- 
              ggbarplot(tb, x=all_of(xaxis), y=List_var[i], fill="Genotype", add = "mean_se", 
            ...
            if (input$task == "fz_day1") { #day1
              if (i < 3) { #freezing/distance
                ...
                Plot <- 
                  ggline(tb1, x=all_of(xaxis), y=List_var[i], color="Genotype", add = "mean_se", alpha=0.7) +
                ...
                Plot <- 
                  ggline(tb2, x=all_of(xaxis), y=List_var[i], color="Genotype", add = "mean_se", 
            ...
            stat.test <- tb %>%
              ...
              ungroup() %>%
              t_test(as.formula(paste0(List_var[i], "~Genotype")))
            stat.test$x.position <- max(tb[,all_of(xaxis)],na.rm=TRUE)*0.8
            ...
            Plot <- 
              ggline(tb, x=all_of(xaxis), y=List_var[i], color="Genotype", add = "mean_se", alpha=0.7) +
            ...
            stat.test <- tb %>%
              anova_test(dv=List_var[i], wid="MouseID", within=all_of(xaxis), between="Genotype")
            # group_by_("MouseID","Genotype") %>%
            # summarize_(.dots = setNames(paste0('mean(', List_var[i], ')'), List_var[i])) %>%
            # ungroup() %>%
            # t_test(as.formula(paste0(List_var[i], "~Genotype")))
            ...
            stat.test$x.position <- max(tb[,all_of(xaxis)],na.rm=TRUE)*0.7
            stat.test$y.position <- max(tb[,List_var[i]],na.rm=TRUE)*0.85
            
            if (input$task == "of"){
              Plot <- 
                ggline(tb, x=all_of(xaxis), y=List_var[i], color="Genotype", add = "mean_se", numeric.x.axis = TRUE) +
                annotate("text", label=paste(expression("Repeated measures ANOVA, \nP ="), stat.test$p), x =  stat.test$x.position, y = stat.test$y.position, size=4) +
                scale_color_viridis_d(label=labels) +
                scale_x_continuous(limits = c(NA,max(tb[,all_of(xaxis)],na.rm=TRUE)),
                                   breaks = seq(max(tb[,all_of(xaxis)],na.rm=TRUE) %% 2, max(tb[,all_of(xaxis)],na.rm=TRUE), by = as.integer(max(tb[,xaxis],na.rm=TRUE)/24))) +
                xlab(xlab) +
                ylab(List_label[i]) +
                theme_classic() + 
                theme(text = element_text(size = 15),
              ...
              Plot <- 
                ...
                scale_color_viridis_d(label=labels) +
                scale_x_continuous(limits = c(NA,max(tb[,all_of(xaxis)],na.rm=TRUE)),
                                   breaks = seq(max(tb[,all_of(xaxis)],na.rm=TRUE) %% 2, max(tb[,all_of(xaxis)],na.rm=TRUE), by = as.integer(max(tb[,all_of(xaxis)],na.rm=TRUE)/6))) +
                xlab(xlab) +
                ylab(List_label[i]) +
                theme_classic() + 
                theme(text = element_text(size = 15),
...
ui <- fluidPage(
  ...
  mainPanel(
    tabsetPanel(
      ...
      Version 0.2 (2021/10/6 of the <b>Mouse Phenotype Database App</b><br/>
